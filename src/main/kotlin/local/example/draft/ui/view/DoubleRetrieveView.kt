...
class DoubleRetrieveView : VerticalLayout(), View {

    private val label: Label
    private val doubleRetrievedLabel: Label
    ...
    init {
        ...
        this.doubleRetrievedLabel = Label("click and view here")
        this.content = VerticalLayout(doubleRetrievedLabel, retrieve)
        this.service = RestRetrieve()
        /* action click */
        this.retrieve.addClickListener { listener ->
            if (null != listener) {
                try {
                    this.doubleRetrievedLabel.value = service.doubleUriResponse()
                } catch (e: Exception) {
    ...
    companion object {
        private const val serialVersionUID = 5416211765189940551L
        const val TITLE_VIEW = "Double Retrieve"
        const val VIEW_NAME = "doubleretrieve"
    }
}

...
class LongRetrieveView : VerticalLayout(), View {

    private val label: Label
    private val longRetrieveLabel: Label
    ...
    init {
        ...
        this.longRetrieveLabel = Label("click and view here")
        this.content = VerticalLayout(longRetrieveLabel, retrieve)
        this.service = RestRetrieve()
        /* action click */
        this.retrieve.addClickListener { listener ->
            if (null != listener) {
                try {
                    this.longRetrieveLabel.value = service.longUriResponse()
                } catch (e: Exception) {
    ...
    companion object {
        private const val serialVersionUID = 8575272719853048731L
        const val TITLE_VIEW = "Long Retrieve"
        const val VIEW_NAME = "longretrieve"
    }
}
